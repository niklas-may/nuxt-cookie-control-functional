...
module.exports = function cookies(_options) {
  const defaultOptions = {
    ...
    componentFunctional: path.resolve(
      __dirname,
      '../components/CookieControlFunctional.vue'
    ),
  }
  ...
  if (options.css)
    this.options.css.push(path.resolve(__dirname, '../lib/styles.scss'))
  ...
  if (options.blockIframe) {
    const blockIframe = {
      name: 'functional',
      initialState:
        options.blockIframe.initialState !== undefined
          ? options.blockIframe.initialState
          : true,
    }
  ...
  try {
    if (require.resolve('nuxt-purgecss').length > 0) {
      if (this.options.purgeCSS) {
        if (this.options.purgeCSS.whitelistPatternsChildren)
          this.options.purgeCSS.whitelistPatternsChildren.push(/cookieControl/)

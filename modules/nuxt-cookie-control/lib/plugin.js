...
import CookieControlFunctional from <%= serialize(options.componentFunctional) %>;
...
export default(context, inject) =>{
  ...
  const clearCookies = () =>{
    ...
    if(disabled.length > 0){
      disabled.forEach(c => {
        if(c.declined) callFunction(c.declined);
      ...
      cookies.enabled.forEach(c =>{
        if(c.src){
          ...
          script.addEventListener('load', () =>{
            if(c.accepted) callFunction(c.accepted);
          })
        }
      })
    }
  ...
  const callFunction = fn => {
    const isString = typeof fn === 'string'
  ...
  Vue.component('CookieControlFunctional', CookieControlFunctional);
}